...
variable "security_group_eb_name" {
  type        = string
  default     = "eb-tasks-sg"
  description = "Elastic Beanstalk security group name"
}

variable "security_group_eb_tasks_description" {
  type        = string
  default     = "allow inbound access from the Elastic Beanstalk ALB only"
  description = "Elastic beanstalk tasks security group description"
}
